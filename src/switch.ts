...
/** RPC 结果最小契约（结构匹配 connection 服务的 RpcResult）。 */
...
/** host 端 RPC 注册器的最小契约。 */
interface RpcHost {
  handle(channel: string, handler: RpcHandler, options: { authority: 'trusted-host' | 'loopback' }): () => Promise<void>
}
...
export class SwitchIntent {
  private pending: { sessionId: string; seq: number } | undefined = undefined
  private seq = 0
 ...
 */
export function registerSwitchRpc(ctx: Context, intent: SwitchIntent): void {
  ctx.inject(['connection'], (connCtx) => {
    const connection = connCtx.get('connection') as { rpc?: RpcHost } | undefined
