...
function matchesSessionQuery(row: { session_id: string; title?: string; cwd?: string; workspace_title?: string }, query: string): boolean {
  const needle = query.toLowerCase()
  const fields: Array<string | undefined> = [row.session_id, row.title, row.cwd, row.workspace_title]
