...
var SESSION_TOOL_PRESENTATIONS = {
  ...
  list_sessions: {
    ...
    summarize: (args) => {
      ...
      if (workspace !== void 0) return archived ? `\u5DE5\u4F5C\u533A ${workspace}\uFF08\u542B\u5DF2\u5F52\u6863\uFF09` : `\u5DE5\u4F5C\u533A ${workspace}`;
...
function apply(ctx) {
  ...
  if (rpc?.call === void 0) return;
  let disposed = false;
  let timer;
  const poll = async () => {
    try {
      const result = await rpc.call("/session-tool", "switch/poll", {});
      const sessionId = result?.ok === true ? result.value?.sessionId : void 0;
